...
const useFirebase = () => {
    ...
    signInWithPopup(auth, gAuthProvider)
      .then((result) => {
        ...
        saveUser(
          ...
          result.user.photoURL || "https://i.ibb.co/4NM5vPL/Profile-avatar-placeholder-large.png",
          "PUT"
        );
  ...
  const createAccountWithEmailPassword = (email, password, name) => {
    ...
    createUserWithEmailAndPassword(auth, email, password)
      .then(async (userCredential) => {
        ...
        setUserProfile(auth, name, "https://i.ibb.co/4NM5vPL/Profile-avatar-placeholder-large.png");
      })
      .catch((err) => {
  ...
  // SET USER PROFILE
  const setUserProfile = (auth, name, photoURL) => {
    setIsLoading(true);
  ...
  const saveUser = async (uid, email, displayName, photoURL, method) => {
    const user = { uid, email, displayName, photoURL };
    await fetch("https://agroveg.herokuapp.com/users", {
      method: method,
      headers: {
  ...
  const getUserRole = async (uid) => {
    // setIsLoading(true);
    await fetch(`https://agroveg.herokuapp.com/users/${uid}`)
      .then((res) => res.json())
      .then((userDb) => {
        if (userDb?.role) {
